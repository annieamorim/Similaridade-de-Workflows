...
def dfs_order_using_nx(graph):
    visited = set()
    order = []

    for node in graph.nodes():
        if node not in visited:
            dfs_tree = nx.dfs_tree(graph, node)
            for dfs_node in dfs_tree.nodes():
                visited.add(dfs_node)
                order.append(graph.nodes[dfs_node]['label'])
...
def calculate_similarity(ref_graph, other_graph, ref_order):
    other_order = dfs_order_using_nx(other_graph)
...
ref_order = dfs_order_using_nx(ref_workflow)

similarities = {}
for i, workflow in enumerate(workflows):
    if i != ref_index:
        order = dfs_order_using_nx(workflow)
...
print(f"Node sequence of Workflow {most_similar_workflow + 1} ({filenames[most_similar_workflow]}): {dfs_order_using_nx(workflows[most_similar_workflow])}")
